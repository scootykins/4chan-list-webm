...
function getWebmList (url, callback, options) {
  ...
  var parsedUrl = /http:\/\/boards.4chan.org\/(.*)\/thread\/(.*)\.json/.exec(url)
  var board = parsedUrl[1];
  var threadNo = parsedUrl[2];

  var apiUrl = function (b, t) { return (protocol + "://a.4cdn.org/" + b + "/thread/" + t + ".json"); }
  var getWebmUrl = function (tim) { return ("http://i.4cdn.org/wsg/" + tim + ".webm"); }
  var isWebm = function (x) { return x.ext === '.webm'; }
  var reducer = function (acc, cur) { return acc.concat([{filename: cur.filename, url: getWebmUrl(cur.tim)}]); }
